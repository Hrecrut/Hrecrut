...
app.post('/api/crm',async(req,res)=>{try{const id=Number(req.body?.company_id);if(!id)return res.status(400).json({error:'company_id requis'});const r=await q(`INSERT INTO crm(company_id) VALUES($1) ON CONFLICT(company_id) DO NOTHING RETURNING id`,[id]);if(r.rows[0])await q(`INSERT INTO crm_events(company_id,type,note) VALUES($1,'statut','Ajouté au CRM')`,[id]);res.json({ok:true,created:!!r.rows[0]})}catch(e){fail(res,e)}});
app.put('/api/crm/:id',async(req,res)=>{try{const id=Number(req.params.id);const old=(await q('SELECT * FROM crm WHERE company_id=$1',[id])).rows[0];if(!old)return res.status(404).json({error:'Entreprise absente du CRM'});const m={...old,...req.body};if(!STAGES.includes(m.stage))return res.status(400).json({error:'Statut invalide'});await q(`UPDATE crm SET stage=$2,notes=$3,next_followup=$4,contact_name=$5,contact_phone=$6,contact_email=$7,updated_at=now() WHERE company_id=$1`,[id,m.stage,m.notes||'',m.next_followup||null,m.contact_name||'',m.contact_phone||'',m.contact_email||'']);if(m.stage!==old.stage)await q(`INSERT INTO crm_events(company_id,type,note) VALUES($1,'statut',$2)`,[id,`${old.stage} → ${m.stage}`]);res.json({ok:true})}catch(e){fail(res,e)}});
app.get('/api/crm/:id/events',async(req,res)=>{try{res.json((await q('SELECT * FROM crm_events WHERE company_id=$1 ORDER BY created_at DESC LIMIT 50',[req.params.id])).rows)}catch(e){fail(res,e)}});
app.post('/api/crm/:id/events',async(req,res)=>{try{const id=Number(req.params.id);const type=['appel','email','rdv','note'].includes(req.body?.type)?req.body.type:'note';await q('INSERT INTO crm_events(company_id,type,note) VALUES($1,$2,$3)',[id,type,String(req.body?.note||'')]);if(type==='appel')await q(`UPDATE crm SET last_call_at=now(),updated_at=now(),stage=CASE WHEN stage IN ('Nouveau prospect','À appeler') THEN 'Appelé' ELSE stage END WHERE company_id=$1`,[id]);res.json({ok:true})}catch(e){fail(res,e)}});
app.delete('/api/crm/:id',async(req,res)=>{try{await q('DELETE FROM crm WHERE company_id=$1',[req.params.id]);await q('DELETE FROM crm_events WHERE company_id=$1',[req.params.id]);res.json({ok:true})}catch(e){fail(res,e)}});
