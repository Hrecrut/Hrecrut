...
function showCandidateForm(candidate=null){
    ...
    <article class="settings-card">
      ...
      <div class="candidate-form">
        ...
        <label>
          Compétences
          <textarea id="cSkills"
            placeholder="Ex : maintenance industrielle, électrotechnique, automatisme, pneumatique..."
        ...
        <label>
          ...
          <input
            id="cAvailability"
            placeholder="Ex : immédiate / 1 mois / disponible..."
      ...
      <p id="candidateSaved"></p>

    </article>
  `;

  if(c.contract_type){
    $('#cContract').value=c.contract_type;
  }
  ...
  $('#saveCandidate').onclick=async()=>{

    const payload={
      first_name:$('#cFirstName').value.trim(),
      last_name:$('#cLastName').value.trim(),
      title:$('#cTitle').value.trim(),
      location:$('#cLocation').value.trim(),
      department:$('#cDepartment').value.trim(),
      skills:$('#cSkills').value.trim(),
      experience:$('#cExperience').value.trim(),
      salary_min:$('#cSalaryMin').value
        ? Number($('#cSalaryMin').value)
        : null,
      salary_max:$('#cSalaryMax').value
        ? Number($('#cSalaryMax').value)
        : null,
      contract_type:$('#cContract').value,
      availability:$('#cAvailability').value.trim(),
      source:$('#cSource').value.trim()||'manuel',
      source_url:$('#cSourceUrl').value.trim(),
      notes:$('#cNotes').value.trim(),
      active_search:$('#cActive').checked
    };
